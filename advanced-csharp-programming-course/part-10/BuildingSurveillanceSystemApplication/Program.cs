...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        var securitySurveillanceHub = new SecuritySurveillanceHub();

        var employeeNotify = new EmployeeNotify(new Employee
        {
            Id = 1,
            FirstName = "Bob",
            LastName = "Jones",
        ...
        var employeeNotify2 = new EmployeeNotify(new Employee
        {
            Id = 2,
            FirstName = "Dave",
            LastName = "Kendal",
        ...
        var securityNotify = new SecurityNotify();
        ...
        securitySurveillanceHub.ConfirmExternalVisitorEntersBuilding(1, "Andrew", "Jackson", "The Company",
            "Contractor", DateTime.Parse("12 May 2020 11:00"), 1);
        securitySurveillanceHub.ConfirmExternalVisitorEntersBuilding(2, "Jane", "Davidson", "Another Company", "Lawyer",
            DateTime.Parse("12 May 2020 12:00"), 2);
...
public class EmployeeNotify : Observer
{
    ...
    public override void OnCompleted()
    {
        ...
        foreach (var externalVisitor in _externalVisitors)
        {
            ...
            Console.WriteLine(
                $"{externalVisitor.Id,-6}{externalVisitor.FirstName,-15}{externalVisitor.LastName,-15}{externalVisitor.EntryDateTime.ToString("dd MMM yyyy hh:mm:ss"),-25}{externalVisitor.ExitDateTime.ToString("dd MMM yyyy hh:mm:ss tt"),-25}");
        }
    ...
    public override void OnNext(ExternalVisitor value)
    {
        ...
        if (externalVisitor.EmployeeContactId == _employee.Id)
        {
            ...
            if (externalVisitorListItem == null)
            {
                ...
                Console.WriteLine(
                    $"{_employee.FirstName + " " + _employee.LastName}, your visitor has arrived. Visitor ID({externalVisitor.Id}), FirstName({externalVisitor.FirstName}), LastName({externalVisitor.LastName}), entered the building, DateTime({externalVisitor.EntryDateTime.ToString("dd MMM yyyy hh:mm:ss")})");
...
public class SecurityNotify : Observer
{
    ...
    public override void OnNext(ExternalVisitor value)
    {
        ...
        if (externalVisitorListItem == null)
        {
            ...
            Console.WriteLine(
                $"Security notification: Visitor Id({externalVisitor.Id}), FirstName({externalVisitor.FirstName}), LastName({externalVisitor.LastName}), entered the building, DateTime({externalVisitor.EntryDateTime.ToString("dd MMM yyyy hh:mm:ss tt")})");
        ...
        else
        {
            if (externalVisitor.InBuilding == false)
            {
                ...
                Console.WriteLine(
                    $"Security notification: Visitor Id({externalVisitor.Id}), FirstName({externalVisitor.FirstName}), LastName({externalVisitor.LastName}), exited the building, DateTime({externalVisitor.ExitDateTime.ToString("dd MMM yyyy hh:mm:ss tt")})");
                Console.WriteLine();
            }
        }
    }
...
public class SecuritySurveillanceHub : IObservable<ExternalVisitor>
{
    ...
    public void ConfirmExternalVisitorEntersBuilding(int id, string firstName, string lastName, string companyName,
        string jobTitle, DateTime entryDateTime, int employeeContactId)
    {
    ...
    {
        if (_externalVisitors.Any(e => e.InBuilding)) return;
